...
def eval_policy(args, iter, video: VideoRecorder, logger: Logger, 
                policy, env_name, seed, mean, std, std_obs_noise=0, seed_offset=100,
                eval_episodes=10):
    ...
    for _ in range(eval_episodes):
        ...
        while not done:
            ...
            state += np.random.normal(0, std_obs_noise, state.shape[1]).reshape(1,-1)
    ...
    d4rl_score_list = [eval_env.get_normalized_score(return_) * 100 for return_ in returns]

    logger.log('eval/offline lengths_mean', np.mean(lengths), iter)
    logger.log('eval/offline returns_mean', np.mean(returns), iter)
    logger.log('eval/offline returns_var', np.var(returns), iter)
    logger.log('eval/offline d4rl_score', d4rl_score, iter)
    logger.log('eval/offline d4rl_score_var', np.var(d4rl_score_list), iter)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--std_obs_noise", default=0, type=float)    # Noise for observations(increase stochasticity)
    parser.add_argument("--seabo_enable", default=True)   # Whether or not enable SEABO
    ...
    if args.seabo_enable is True: # The only difference is whether or not enable the reward
    ...
    for t in trange(int(args.max_timesteps)):
        ...
        if (t + 1) % args.eval_freq == 0:
            ...
            d4rl_score = eval_policy(args, t+1, video, logger, policy, args.env,
                                     args.seed, mean, std,  args.std_obs_noise, eval_episodes=eval_episodes)
            if args.save_model:
